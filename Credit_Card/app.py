...
with tab4:
    ...
    with col1:
        ...
        if tpl_type == "CSV":
            st.download_button("📥 Structure Blueprint (CSV)",  tpl_data.to_csv(index=False),                 "loan_template.csv",  width='stretch')
        elif tpl_type == "JSON":
            ...
            st.download_button("📥 Structure Blueprint (SQL)",  sql_txt,                                       "loan_template.sql",  width='stretch')
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col3:
        ...
        if st.session_state.scan_done:
            ...
            if exp_fmt == "CSV":
                st.download_button("💾 Export CSV Logs",    st.session_state.result_df.to_csv(index=False),                         "loan_report.csv",  width='stretch')
            elif exp_fmt == "JSON":
                ...
                st.download_button("💾 Export SQL (Top 1k rows)", sql_out,                                                          "loan_report.sql",  width='stretch')
        else:
...
if st.session_state.scan_done:
    ...
    try:
        ...
        st.dataframe(display_df, width='stretch', height=550)
